...
class Skillshare():
    ...
    # CLASS ATTRIBUTES - that are common to all INSTANCE/OBJECTS
    cookie = ''
    ...
    def __init__(self, course_url, from_user_id):
        ...
        self.file_name = ''
        self.size = ''
        self.downloaded_location = ''
        # self.anonfile_url = ''
        # self.pixeldrain_url = ''
        # self.gofile_url = None
        # self.gofile_folder_url = None
    
    @classmethod
    def initialise(cls, SKILLSHARE_COOKIE, course_url, from_user_id):
        cls.cookie = SKILLSHARE_COOKIE
        return cls(course_url=course_url, from_user_id=from_user_id)
    ...
    def download_video(fpath, video_id):
        meta_url = 'https://edge.api.brightcove.com/playback/v1/accounts/{account_id}/videos/{video_id}'.format(
            account_id = Skillshare.brightcove_account_id,
            video_id = video_id,
        )
        ...
        meta_res = scraper.get(
            ...
            headers={
                ...
                'Accept': 'application/json;pk={}'.format(Skillshare.pk),
    ...
    def fetch_course_data_by_class_id(self):
        url = 'https://api.skillshare.com/classes/{}'.format(self.course_id)
        scraper = cloudscraper.create_scraper(
            browser={
    ...
    def download_course_by_id(self):
        data = self.fetch_course_data_by_class_id()

        if 'vanity_username' in data['_embedded']['teacher']:
            self.teacher = data['_embedded']['teacher']['vanity_username']

        if not self.teacher:
            self.teacher = data['_embedded']['teacher']['full_name']

        if not self.teacher:
            raise Exception('Failed to read teacher name from data')

        if Skillshare.is_unicode_string(self.teacher):
            self.teacher = self.teacher.encode('ascii', 'replace')

        title = slugify(data['title'])

        if Skillshare.is_unicode_string(title):
        ...
        for s in data['_embedded']['sessions']['_embedded']['sessions']:
            ...
            if Skillshare.is_unicode_string(s_title):
        ...
        self.downloaded_location = base_path
        self.title = slugify(title)

        return print(f'Finished Downloading - {title}')

    def download_course_by_url(self):
        m = re.match(r'https://www.skillshare.com/classes/.*?/(\d+)', self.course_url)
        if m is None:
            m = re.match(r'https://www.skillshare.com/[a-zA-Z]+/classes/.*?/(\d+)', self.course_url)

        if m is None:
            raise Exception('Failed to parse class ID from URL')

        self.course_id = m.group(1)
        self.download_course_by_id()
